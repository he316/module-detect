...
{
  cat("round ",i,"\n")
  clusterName <- Leiden_clustering_size[i,1]# = 第i個cluster的檔案名稱
  clusterCellNumber <- Leiden_clustering_size[i,2]# = 第i個cluster的cell數量
  ...
  par(mfrow = c(1,2))
  cex1 = 0.9
  # ??????是Soft threshold (power)，??????是??????度网???的???估??????，???值越高，
  # 网???越符合??????度特征 (non-scale)
  ...
  # 如果??????果不???意，???可以recutBlockwiseTrees，???省???算??????
  ## SAVE!
  ...
  #可以不用跑，也可以跑，不會太久
  # module eigengene, 可以???制??????，作???每???模???的基因表?????????的展示
  MEs = net$MEs
  if(ncol(MEs)>2)
  {
    #防呆,Module過少就不會執行,若module只有一個的話dendrogram畫不出來
    ### 不需要重新???算，改下列名字就好
    ### 官方教程是重新???算的，起始可以不用???么麻???
    MEs_col = MEs
    colnames(MEs_col) = paste0("ME", labels2colors(as.numeric(str_replace_all(colnames(MEs),"ME",""))))
    MEs_col = orderMEs(MEs_col)
    
    # 根据基因???表???量???行聚???所得到的各模??????的相???性???
    # marDendro/marHeatmap ???置下、左、上、右的???距
    ## SAVE!
    if(ncol(MEs)>3)
    {
  ...
  #這步可以不用跑，很花時間，有需要圖再跑
  # 如果采用分步???算，或???置的blocksize>=???基因???，直接load???算好的TOM???果
  # 否???需要再???算一遍，比???耗?????????
  ...
  # threshold 默??????0.5, 可以根据自己的需要???整，也可以都???出后在
  # cytoscape中再???整
